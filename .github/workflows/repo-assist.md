...
2. For each PR: fix CI failures caused by your changes by pushing updates.
3. **Do not rebase or merge main into existing Repo Assist PR branches.** History rewrites, force-pushes, and `git merge main` are all forbidden — they litter the PR with unrelated commits. If the branch is behind main or has a merge conflict, **leave a comment** explaining the situation and let maintainers handle integration.
4. Only push changes that are strictly your own new fix commits on top of the existing branch tip.
5. If you've retried fixing CI failures multiple times without success, comment and leave for human review.
6. Do not push updates for infrastructure-only failures  -  comment instead.
6. Update memory.
